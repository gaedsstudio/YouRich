...
def test_overlapping_ytd_rows_do_not_create_complete_ttm() -> None:
    ...
    assert [str(fact.period_class) for fact in selection.facts] == [
        "YTD_9M",
        "ANNUAL",
        "YTD_9M",
    ]
...
def quarter_fact(value: int, fp: str, start: str, end: str) -> dict[str, Any]:
    return {
        "val": value,
        "fy": 2026,
        "fp": fp,
...
def annual_fact(value: int, fy: int, start: str, end: str) -> dict[str, Any]:
    return {
        "val": value,
        "fy": fy,
        "fp": "FY",
